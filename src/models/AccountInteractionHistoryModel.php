...
class AccountInteractionHistoryModel {
    ...
    public function insertInteraction($accountId, $contactId, $userId, $targetListId, $nextContactDate, $notes, $outcome, $contactMethod) {
        $sql = "INSERT INTO account_interaction_history (account_id, contact_id, user_id, target_list_id, contacted_at, next_contact_date, notes, outcome, contact_method)
                VALUES (?, ?, ?, ?, NOW(), ?, ?, ?, ?)";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param('iiiissss', $accountId, $contactId, $userId, $targetListId, $nextContactDate, $notes, $outcome, $contactMethod);
        $stmt->execute();
    }
}
